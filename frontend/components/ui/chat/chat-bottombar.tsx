      ...
      <AnimatePresence initial={false}>
        ...
        >
          <Textarea
            ...
            className=" w-full border min-h-0 rounded-full flex items-center h-9 resize-none overflow-hidden bg-background"
          ></Textarea>
          {/* <Textarea
    autoComplete="off"
    value={message}
...
>
    {isLink(message) ? (
        <Link href={message} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
            {message}
        </Link>
    ) : (
        message
        ...
        {message.trim() ? (
          <Link
            ...
            className={cn(
              buttonVariants({ variant: "ghost", size: "icon" }),
              "h-9 w-9",
              "dark:bg-muted dark:hover:bg-muted dark:hover:text-white shrink-0"
            )}
            onClick={handleSend}
          >
